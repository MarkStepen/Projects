...
#define ASUCHAR(x,ofs)  (*((u8  *)&x+ofs))					// x为要拆的数,0fs为第几位
#define ASU16(x,ofs)  	(*((u16  *)&x+ofs))	                // x为要拆的数,0fs为第几位
...
void TIM3_Configuration(u16 arr,u16 psc);  // 定时器3的PWM输出
...
void DAC1_Set_VOL(u16 vol) ;                  // DAC函数         
...
#define RECV_BUFFER_SIZE  1100   //从串口 收发发缓冲

#define ASUCHAR(x,ofs)  (*((u8  *)&x+ofs))					//x为要拆的数,0fs为第几位

extern u32   Animal_Heat;                          // 体温变量
